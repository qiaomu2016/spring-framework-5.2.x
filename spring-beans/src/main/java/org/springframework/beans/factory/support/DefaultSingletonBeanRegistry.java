...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * 一级缓存 单例池 主要存放最终形态的单例bean；我们一般获取一个bean都是从这个缓存中获取；
	 * 需要说明的是并不是所有单例bean都存在这个缓存当中，有些特殊的单例bean不存在这个缓存当中
	 */
	private final Map<String, Object> singletonObjects = new ConcurrentHashMap<>(256);

	/**
	 * Cache of singleton factories: bean name to ObjectFactory.
	 * 三级缓存 他的作用主要是为了产生一个对象；因为这个缓存当中存放的是一个工厂；可以产生特定对象；
	 * 程序员可以去扩展BeanPostProcessor来定制这个工厂产生对象的过程；比如AOP就是扩展了这个工厂的产生过程；从而完成完整的aop功能；
	 * 如果没有这个缓存，那么极端情况下会出现循环依赖注入的bean不是一个完整的bean，或者说是一个错误的bean
	 */
	private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);

	/**
	 * Cache of early singleton objects: bean name to bean instance.
	 * 二级缓存 主要存放的是过渡bean；也就是从三级缓存当中产生出来的对象；他的作用是防止在多级循环依赖的情况下重复从三级缓存当中创建对象；
	 * 因为三级缓存当中创建对象是需要牺牲一定得性能；有了这个缓存可以一定程度上提高效率
	*/
	private final Map<String, Object> earlySingletonObjects = new ConcurrentHashMap<>(16);

	/**
	 * 按顺序存放已经注册的Singleton Bean（已经被spring给初始化好了的）
	...
	protected void addSingletonFactory(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			if (!this.singletonObjects.containsKey(beanName)) { // 不存在单例池中
		...
		// 这里，我们已经通过 #getSingleton(String beanName, boolean allowEarlyReference) 方法，看到他们是如何配合的。详细分析该方法之前，提下其中的
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		// isSingletonCurrentlyInCreation 判断beanName是否正在被创建
			...
			if (singletonObject == null && allowEarlyReference) {
				synchronized (this.singletonObjects) {
					...
					if (singletonObject == null) {
						singletonObject = this.earlySingletonObjects.get(beanName);
						if (singletonObject == null) {
							ObjectFactory<?> singletonFactory = this.singletonFactories.get(beanName); // 三级缓存获取
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			Object singletonObject = this.singletonObjects.get(beanName);
			if (singletonObject == null) { //  为空，开始加载过程
				...
				// 判断beanName有没有被排除，如果没有排除，则将beanName添加到singletonsCurrentlyInCreation的Set集合中，表示beanName对应的bean正在创建中
				beforeSingletonCreation(beanName); // 标识bean正在创建中
				...
				if (newSingleton) {
					// 把bean添加至singletonObject、registeredSingletons集合中
