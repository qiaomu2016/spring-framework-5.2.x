...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		if (targetSource != null) {
			...
			this.proxyTypes.put(cacheKey, proxy.getClass()); // 对生成的代理对象进行缓存
			return proxy; // 直接返回生成的代理对象
		}
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		proxyFactory.copyFrom(this); 	// 复制proxyTargetClass，exposeProxy等属性
		...
		if (proxyFactory.isProxyTargetClass()) {
			...
			if (shouldProxyTargetClass(beanClass, beanName)) {	// 判断目标类是否设置了preserveTargetClass属性
				proxyFactory.setProxyTargetClass(true);
			}
			else {
				evaluateProxyInterfaces(beanClass, proxyFactory);	// 判断目标类是否实现了相关接口
			}
		}
		...
		customizeProxyFactory(proxyFactory); // 提供的hook方法，供子类实现以实现对代理工厂的定制
